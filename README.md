...
当前正式对外域名：`https://www.liutongxue.com.cn`

默认公开访问仅保留这一正式域名；裸域 `https://liutongxue.com.cn` 应跳转到 `https://www.liutongxue.com.cn`。仓库内已移除 GitHub Pages 部署路径，也不再把 `vercel.app` / 其他静态出口作为默认公开入口。
...
- 在 `www.liutongxue.com.cn` 同域访问时，聊天页默认直接请求当前域名下的 `/api/chat`。
