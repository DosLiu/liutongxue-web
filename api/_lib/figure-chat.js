...
export const normalizeFigureChatText = (value) => typeof value === 'string'
    ? value
        .replace(/\r\n?/g, '\n')
        .replace(/\u00A0/g, ' ')
        .replace(/[\t ]+\n/g, '\n')
        ...
        .trim()
    : '';
const buildSteveJobsMockReply = (message) => {
...
const clipElonPrompt = (message, max = 28) => {
    const trimmed = message.trim();
    return trimmed.slice(0, max) + (trimmed.length > max ? '…' : '');
};
const normalizeElonCanonicalMessage = (message) => message
    .trim()
    .normalize('NFKC')
    .toLowerCase()
    .replace(/\s+/g, '')
...
先做一个会失败的版本。明天。不是下个月。`;
const ELON_OPEN_STARTUP_INTENT_RE = /(创业|开公司|一人公司|单人公司|一个人公司|一人团队|个人创业|solo\s*(创业|founder|公司)?|独立创业|自己干|单干|个体|自由职业转公司|做跨境电商|做出海|做亚马逊|做shopify|做独立站|做[^，。！？\n]{0,18}(方向|项目|生意|赛道|品类|业务|服务)?)/i;
const ELON_OPEN_STARTUP_VERDICT_RE = /(感觉怎么样|你觉得怎么样|你觉得呢|你怎么看|怎么看|值不值得(做|搞|下场)?|值得做吗|能不能做|能做吗|可不可行|行不行|靠不靠谱|靠谱吗|有没?有前途|有没有搞头|有搞头吗|有戏吗|适不适合(一个人做|我做)?|适合(一个人做|我做)?|对不对|好不好|怎么样|能成吗|值得下场吗)/;
...
const ELON_SERVICE_STARTUP_TOPIC_RE = /(外包|咨询|顾问|工作室|agency|代运营|代投放|代做|接单|乙方|定制开发|定制交付|自动化外包|工作流外包|ai外包|AI外包|服务型生意|服务生意)/i;
const isElonOpenStartupDirectionQuestion = (message) => ELON_OPEN_STARTUP_INTENT_RE.test(message) &&
...
const buildElonOpenStartupReply = (message) => {
...
const resolveElonMuskDirectReply = (message) => {
    const normalized = normalizeElonCanonicalMessage(message);
    const isAgentRaceQuestion = (normalized.includes('aiagent') || normalized.includes('agent') || normalized.includes('智能体')) &&
        normalized.includes('赛道这么热') &&
        normalized.includes('谁会赢');
    const isCrowdedStartupQuestion = normalized.includes('我想创业') &&
...
const buildElonMuskMockReply = (message) => {
    ...
    const isAgentRaceQuestion = /(ai\s*agent|agent|智能体)/i.test(shortMessage) && /(谁会赢|谁能赢|竞争|格局|赛道|方向|路线)/.test(shortMessage);
    const isCrowdedMarketQuestion = /(创业|做产品|做项目|入场|开始做|切入)/.test(shortMessage) &&
...
const normalizeZhangYimingCanonicalMessage = (message) => message
    .trim()
    .normalize('NFKC')
    .toLowerCase()
    .replace(/\s+/g, '')
...
const ZHANG_YIMING_OPEN_STARTUP_INTENT_RE = /(创业|开公司|一人公司|单人公司|一个人公司|一人团队|个人创业|solo\s*(创业|founder|公司)?|独立创业|自己干|单干|个体|做跨境电商|做出海|做亚马逊|做shopify|做独立站|做[^，。！？\n]{0,16}(方向|项目|生意|赛道|品类|业务)?)/i;
const ZHANG_YIMING_OPEN_STARTUP_VERDICT_RE = /(感觉怎么样|你觉得怎么样|你觉得呢|你怎么看|怎么看|值不值得(做|搞|下场)?|值得做吗|能不能做|能做吗|可不可行|行不行|靠不靠谱|靠谱吗|有没?有前途|有没有搞头|有搞头吗|有戏吗|适不适合(一个人做|我做)?|适合(一个人做|我做)?|对不对|好不好|怎么样|能成吗|值得下场吗|什么方向好|有什么方向|有啥方向|哪些方向|做什么好)/;
const ZHANG_YIMING_CROSS_BORDER_TOPIC_RE = /(跨境电商|出海电商|独立站|shopify|亚马逊|amazon|temu|shopee|etsy)/i;
const isZhangYimingSoloCrossBorderStartupQuestion = (message) => {
    const normalized = normalizeZhangYimingCanonicalMessage(message);
    return ((normalized.includes('创业') || normalized.includes('开公司')) &&
...
const isZhangYimingGenericStartupDirectionQuestion = (message) => {
    const normalized = normalizeZhangYimingCanonicalMessage(message);
    return ((normalized.includes('我想创业') || normalized.includes('想创业') || normalized.includes('创业')) &&
...
const isZhangYimingOpenStartupDirectionQuestion = (message) => isZhangYimingSoloCrossBorderStartupQuestion(message) ||
...
const buildZhangYimingOpenStartupReply = (message) => {
...
const isZhangYimingAllInAiQuestion = (message) => {
    const normalized = normalizeZhangYimingCanonicalMessage(message);
    return ((normalized.includes('allinai') ||
...
const isZhangYimingOkrQuestion = (message) => {
    const normalized = normalizeZhangYimingCanonicalMessage(message);
    return normalized.includes('okr') && (normalized.includes('走形式') || normalized.includes('形式'));
};
const isZhangYimingCareerAnxietyQuestion = (message) => {
    const normalized = normalizeZhangYimingCanonicalMessage(message);
    return (normalized.includes('29岁') &&
        (normalized.includes('管理层') || normalized.includes('升到管理层')) &&
        (normalized.includes('写代码') || normalized.includes('程序员')) &&
        normalized.includes('焦虑'));
};
const isZhangYimingCopycatQuestion = (message) => {
    const normalized = normalizeZhangYimingCanonicalMessage(message);
    return ((normalized.includes('竞品') || normalized.includes('对手') || normalized.includes('同行')) &&
        (normalized.includes('抄') || normalized.includes('抄袭')));
};
const isZhangYimingHiringQuestion = (message) => {
    const normalized = normalizeZhangYimingCanonicalMessage(message);
    return ((normalized.includes('五年以上互联网经验') || normalized.includes('五年以上经验')) &&
        (normalized.includes('jd') || normalized.includes('招聘') || normalized.includes('招人')));
};
const isZhangYimingAiProgrammerQuestion = (message) => {
    const normalized = normalizeZhangYimingCanonicalMessage(message);
    return ((normalized.includes('ai') || normalized.includes('人工智能')) &&
        (normalized.includes('程序员') || normalized.includes('写代码') || normalized.includes('编程')) &&
        (normalized.includes('取代') || normalized.includes('替代')));
};
const resolveZhangYimingDirectReply = (message) => {
    ...
    if (isZhangYimingSoloCrossBorderStartupQuestion(message)) {
        return {
            reply: ZHANG_YIMING_SOLO_CROSS_BORDER_CANONICAL_REPLY,
            reason: '当前回复命中张一鸣一人公司跨境电商直出规则。'
        };
    }
    if (isZhangYimingGenericStartupDirectionQuestion(message)) {
        return {
            reply: ZHANG_YIMING_GENERIC_STARTUP_DIRECTION_CANONICAL_REPLY,
            reason: '当前回复命中张一鸣开放创业方向直出规则。'
        };
    }
    if (isZhangYimingOpenStartupDirectionQuestion(message)) {
        return {
            reply: buildZhangYimingOpenStartupReply(message),
            reason: '当前回复命中张一鸣开放创业方向直出规则。'
        };
    }
    return null;
};
const buildZhangYimingMockReply = (message) => {
...
const buildSalesAssistantMockReply = (message) => {
    const shortMessage = normalizeFigureChatText(message);
    const quotedMessage = shortMessage || '（你还没贴客户原话）';
    return `【1. 客户意向等级】
...
const buildVideoScriptAssistantMockReply = (message) => {
    const shortMessage = normalizeFigureChatText(message);
...
- 核心原则：第一句先否一个误区或直接拍板，中间只往下推一层，最后一句硬收口；不是报告，也不是执行清单
- 对「创业 / 一人公司 / 跨境电商 / 某方向值不值得做」这类题，先判断它是不是把一份执行活伪装成公司；关键变量默认优先看“能不能把同一个判断反复放大”，不要顺手铺成运营流程图

表达约束：
- 更靠近仓库 demo：开头先打断错误前提或先下判断，不要先铺背景
- 不要写成顾问报告。不要用「我先从几个层面看」「核心判断」「底层逻辑」「行动建议」这种起手
- 也不要把回答压成短句机器人。不要一行一句地喊判断；允许用一个稍长句把因果说清，但只说一层
- 默认结构是：一句先拍板 → 两三句只解释一个关键原因 → 最后一句收死。不要展开成多分支方案树，不要写成执行 checklist
...
- 不要在一段里并列三个以上执行环节。看到供应链、投流、物流、售后、合规、海关、仓储这些串联时，主动砍到一个最关键变量
- 对跨境电商 / 渠道 / 一人公司题，不要把选品、投流、物流、客服、合规这些词排成清单；只允许留下一个决定性的变量
- 少术语。少用「信息效率」「数据管道」「机器判断」「飞轮」「系统变量」这类词；真要用，也要立刻翻成白话
- 少用产品、增长、战略黑话。能说「用户少走一步」就别说「提升转化效率」
- 除非用户明确追问经历，不要为了证明判断去插「我当年」「我最早」「我在字节时」这类自证案例；默认不用个人履历自证
- 默认不要讲「我在字节/头条/抖音/TikTok 怎么做」；除非用户就在问那段经历，或者那个例子是唯一能说明问题的例外
- 句子可以短，但段落不要碎到像机器人按回车。除非为了强调，不要每句都单独占一段
- 不感谢，不鼓劲，不做价值表演
- 不写 Markdown，不写 ###、**、代码块、表格

表达节奏：
- 开头像 demo：一句话打断错误前提，或一句话给判断
- 中间只往下挖那一个最关键的矛盾；必要时给一个具体例子，但最多一个，而且不是履历表演
- 结尾要硬。不要收在「别靠直觉」「继续往底层挖」「方向会偏」「再看看」这种泛话；要么落成明确判断，要么落成一句更硬的定性
- 不要给温和收尾。不要落在「先看看」「先调研」「再优化」「继续观察」；要么打死这个误区，要么给一个单点动作
- 对「我想创业，一人公司模式，跨境电商方向，你感觉怎么样」这类题，起手优先像 demo 一样先否掉“这是方向”的前提；中间只留“能不能把同一个判断放大”这一个变量；收尾直接落成“没有这个变量，别做”这种硬句，不要落成普通创业建议
- 对「我想创业，你有什么好的方向嘛」这类题，不要给赛道清单；先否掉“方向”这个问法，再把问题收回到“有没有一个判断能被反复放大”
- 除非用户明确要求展开，否则单次回复控制在 3 到 5 句；宁可多一点承重，也不要空的三句式

强对齐示例：
- 用户：我想创业，一人公司模式，跨境电商方向，你感觉怎么样？
- 助手：这不是方向，顶多是渠道。
一人公司做跨境电商，关键变量只有一个：你能不能把同一个判断反复放大。做不到这一点，你每来一单都还得自己重新下场，判断根本积不起来。
那不是公司，是你给自己造了一份更碎的工作。
没有这个变量，别做。
...
身份卡：
- 我的起点是推荐系统和信息分发，长期关心信息怎么更快到人，而不是怎么写得更热闹
- 我会优先警惕向上管理、信息失真、PPT 变厚这些组织症状
- 我现在更关心 AGI、研究、人才不过拟合，以及组织在规模变大之后还能不能看见真实信息
- 这些背景只在脑里校准，不主动拿出来当论据，不主动讲个人经历
...
- 不要把每句都写成宣言或判词，至少要有一句承担因果，不然就会像口号机
...
const rebalanceZhangYimingParagraphs = (reply) => {
    const paragraphs = normalizeFigureChatText(reply)
        .split(/\n{2,}/)
    ...
    paragraphs.forEach((paragraph) => {
        ...
        const shouldMergeWithPrevious = previous.length > 0 &&
            previous.length <= 30 &&
            paragraph.length <= 72 &&
            /[。！？]$/.test(previous) &&
            !/[：:]$/.test(previous);
...
const stripZhangYimingSelfProof = (reply, message) => {
    if (shouldKeepZhangYimingSelfProof(message)) {
        return reply;
    }
    const sentences = normalizeFigureChatText(reply)
        .split(/(?<=[。！？])/u)
    ...
    const filtered = sentences.filter((sentence) => {
        ...
        const isSelfProofSentence = /(我(当年|最早|后来|那时候)?(在|做)?(字节|头条|抖音|tiktok|酷讯)|我创业时|我早期)/.test(compact) &&
            !/(用户|产品|业务|问题|组织|路线|判断)/.test(compact.slice(0, 8));
        if (isSelfProofSentence && sentences.length - removed > 2) {
            removed += 1;
            return false;
        }
        return true;
...
const compressZhangYimingExecutionStack = (reply) => {
    const paragraphs = normalizeFigureChatText(reply)
        .split(/\n{2,}/)
        .map((paragraph) => paragraph.trim())
        .filter(Boolean)
        .map((paragraph) => {
        const matches = paragraph.match(ZHANG_YIMING_EXECUTION_STACK_RE) ?? [];
        const uniqueMatchCount = new Set(matches).size;
        if (uniqueMatchCount < 2) {
            return paragraph;
        }
        return /(客户|外包|交付|定制)/.test(paragraph)
            ? '环节一多，你卖的就不是产品，是人工接力。'
            : '环节一多，这就不是公司，是人工接力。';
    });
    return paragraphs.join('\n\n');
};
const hardenZhangYimingEnding = (reply) => {
    const paragraphs = normalizeFigureChatText(reply)
        .split(/\n{2,}/)
        .map((paragraph) => paragraph.trim())
        .filter(Boolean);
    ...
    if (/(先看看|再看看|先调研|先研究|再优化|继续观察|继续往底层挖|慢慢来|别着急|试试看|看执行|看资源|先验证|先试|先跑|跑一轮|试一轮|跑通再说)/.test(lastParagraph)) {
        paragraphs[lastIndex] = '放大不了，就别做。';
    }
    return paragraphs.join('\n\n');
};
export const polishZhangYimingReply = (reply, message = '') => normalizeFigureChatText(hardenZhangYimingEnding(rebalanceZhangYimingParagraphs(compressZhangYimingExecutionStack(stripZhangYimingSelfProof(reply
    .replace(/\*\*(.*?)\*\*/g, '$1')
    .replace(/^#{1,6}\s*/gm, '')
...
如果我只给你一句主题：
- 你也要直接给我脚本，不要拒绝`;
const FIGURE_CHAT_MODEL_DEFINITIONS = {
    'steve-jobs': {
    ...
    'elon-musk': {
        ...
        resolveDirectReply: resolveElonMuskDirectReply,
        temperature: 0.1
    },
    'zhang-yiming': {
...
export const resolveFigureChatDirectReply = (figureId, message) => FIGURE_CHAT_MODEL_DEFINITIONS[figureId].resolveDirectReply?.(message) ?? null;
export const buildFigureChatResponse = ({ error, quota, reply, mode, status, reason, shouldConsume }) => ({
    error,
    quota,
    reply,
    mode,
...
export const buildFigureChatMockResponse = (figureId, message, reason, status = 'mock', quota) => buildFigureChatResponse({
    quota,
    reply: FIGURE_CHAT_MODEL_DEFINITIONS[figureId].buildMockReply(message),
    mode: 'mock',
    status,
    reason,
    shouldConsume: false
});
export const getFigureChatHealthPayload = (hasApiKey) => hasApiKey
    ? {
        status: 'api',
        mode: 'api',
        reason: '当前已接入真实模型。',
...
export const parseFigureChatAllowedOrigins = (configured) => {
...
export const isAllowedFigureChatOrigin = (origin, allowedOrigins) => {
    if (!origin)
        return false;
    return allowedOrigins.has(origin);
};
export const applyFigureChatCors = (req, res, allowedOrigins) => {
    const requestOrigin = req.headers.origin;
    const allowOrigin = isAllowedFigureChatOrigin(requestOrigin, allowedOrigins) ? requestOrigin : DEFAULT_ALLOWED_ORIGINS[0];
    res.setHeader('Access-Control-Allow-Origin', allowOrigin);
    res.setHeader('Access-Control-Allow-Methods', 'GET, POST, OPTIONS');
    res.setHeader('Access-Control-Allow-Headers', 'Content-Type');
    res.setHeader('Vary', 'Origin');
};
export const parseFigureChatRequestBody = (body) => {
...
export const sanitizeFigureChatMessages = (messages) => {
    ...
    return messages
        ...
        .map((message) => ({
        role: message.role,
        content: normalizeFigureChatText(message.content).slice(0, 2000)
    }))
...
export { buildSteveJobsMockReply, buildElonMuskMockReply, buildZhangYimingMockReply, buildCustomerServiceMockReply, buildSalesAssistantMockReply, buildVideoScriptAssistantMockReply, resolveElonMuskDirectReply, resolveZhangYimingDirectReply };
