...
export type RequestBody = {
  figureId?: FigureChatId;
  messages?: IncomingMessage[];
};

type FigureChatModelDefinition = {
  systemPrompt: string;
  buildMockReply: (message: string) => string;
  resolveDirectReply?: (message: string) => FigureDirectReply | null;
  temperature?: number;
};

export const trimTrailingSlash = (value: string) => value.replace(/\/+$/, '');

export const normalizeFigureChatText = (value: unknown) =>
  typeof value === 'string'
    ? value
        .replace(/\r\n?/g, '\n')
        .replace(/\u00A0/g, ' ')
...
const buildCustomerServiceMockReply = (message: string) => {
...
const rebalanceZhangYimingParagraphs = (reply: string) => {
...
const shouldKeepZhangYimingSelfProof = (message: string) =>
  /(张一鸣|字节|头条|抖音|tiktok|酷讯|经历|传记|你自己|你当年|你以前|案例|例子|微博|演讲)/i.test(message);

const stripZhangYimingSelfProof = (reply: string, message: string) => {
  ...
  const filtered = sentences.filter((sentence) => {
    ...
    const isSelfProofSentence =
      /(我(当年|最早|后来|那时候)?(在|做)?(字节|头条|抖音|tiktok|酷讯)|我创业时|我早期)/.test(compact) &&
...
const compressZhangYimingExecutionStack = (reply: string) => {
  const paragraphs = normalizeFigureChatText(reply)
    .split(/\n{2,}/)
...
const hardenZhangYimingEnding = (reply: string) => {
  const paragraphs = normalizeFigureChatText(reply)
    .split(/\n{2,}/)
    .map((paragraph) => paragraph.trim())
    .filter(Boolean);
...
export const polishZhangYimingReply = (reply: string, message = '') =>
  normalizeFigureChatText(
    hardenZhangYimingEnding(
      rebalanceZhangYimingParagraphs(
        compressZhangYimingExecutionStack(
          stripZhangYimingSelfProof(
            reply
              .replace(/\*\*(.*?)\*\*/g, '$1')
              .replace(/^#{1,6}\s*/gm, '')
...
export const getFigureChatModelDefinition = (figureId: FigureChatId) => FIGURE_CHAT_MODEL_DEFINITIONS[figureId];

export const resolveFigureChatDirectReply = (figureId: FigureChatId, message: string) =>
  FIGURE_CHAT_MODEL_DEFINITIONS[figureId].resolveDirectReply?.(message) ?? null;

export const buildFigureChatResponse = ({
  error,
  quota,
  reply,
  mode,
...
export const buildFigureChatMockResponse = (
  figureId: FigureChatId,
  message: string,
  reason: string,
...
export const getFigureChatHealthPayload = (hasApiKey: boolean): FigureChatApiResponse =>
  hasApiKey
    ? {
        status: 'api',
        mode: 'api',
...
export const DEFAULT_ALLOWED_ORIGINS = [
  // 站点来源：Vercel 正式站 + GitHub Pages 镜像
...
export const parseFigureChatAllowedOrigins = (configured?: string) => {
...
export const isAllowedFigureChatOrigin = (origin: string | undefined, allowedOrigins: Set<string>) => {
  if (!origin) return false;
  return allowedOrigins.has(origin);
};

export const applyFigureChatCors = (req: any, res: any, allowedOrigins: Set<string>) => {
...
export const parseFigureChatRequestBody = (body: unknown): RequestBody => {
...
export const sanitizeFigureChatMessages = (messages: unknown) => {
...
export const normalizeFigureChatId = (value: unknown): FigureChatId =>
  value === 'elon-musk'
    ? 'elon-musk'
    : value === 'zhang-yiming'
      ? 'zhang-yiming'
