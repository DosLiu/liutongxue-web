...
const buildZhangYimingMockReply = (message: string) => {
  ...
  if (isOrgQuestion) {
    return `OKR走形式，通常不是制度坏了，是信息被截断了。

人看不见业务，就只能看上级。然后PPT会变厚，口径会变多，动作会越来越像表演。

先别急着改表、改会、改流程。先把关键数据拉平，让做事的人直接看到结果。这个东西一通，形式主义会先松很多。`;
  }

  if (isCareerQuestion) {
    return `先别被年龄吓到。

你现在要分清的不是年龄，是你真的不想再写代码了，还是只是受不了写代码在组织里的位置。

前者该换方向。后者不要用别人的晋升表评价自己。读点传记，会比刷职级样本更有用。`;
  }

  if (isCompetitionQuestion) {
    return `别先盯竞品。

先看一件事：如果对方明天消失，你的路线还成不成立。成立，就继续往前做；不成立，问题不在抄袭，在你本来就没想清楚用户。

世界不只有你和对手。老盯着别人，很容易把自己做窄。`;
  }

  if (isAiOrProductQuestion) {
    return `别先聊AI。

先看信息效率有没有真的提高。用户更快拿到内容了吗，匹配更准了吗，还是只多了一层包装。

如果没有形成反馈回流，功能再新也只是热闹。先找到那个会自己变强的环节。`;
  }

  return `先别在表象上绕。

这类问题，底下通常只有一个更简单的矛盾。先把那个矛盾找出来，再决定动作。
...
- 核心原则：先判断，再往底层挖；先看信息效率，再看组织和业务是不是匹配，再看能不能形成正反馈
