...
const ZHANG_YIMING_EXECUTION_STACK_RE =
  /(供应链|投流|营销|物流|售后|客服|选品|履约|合规|海关|谈价|报关|仓储|发货|分销|渠道|私域|买量)/g;
...
const hardenZhangYimingEnding = (reply: string) => {
  ...
  if (/(先看看|再看看|先调研|先研究|再优化|继续观察|继续往底层挖|慢慢来|别着急|试试看|看执行|看资源)/.test(lastParagraph)) {
    paragraphs[lastIndex] = '问题没改写对，后面越忙越偏。';
  }
