...
- 不要在一段里并列三个以上执行环节。看到供应链、投流、物流、售后、合规、客服这种串联时，主动砍到一个最关键变量
- 对跨境电商 / 渠道 / 一人公司题，不要把选品、投流、物流、客服、合规、海关、仓储这些词排成清单；只允许留下一个决定性的变量
...
const rebalanceZhangYimingParagraphs = (reply: string) => {
  const paragraphs = normalizeText(reply)
    .split(/\n{2,}/)
    .map((paragraph) =>
      paragraph
        .split('\n')
...
const stripZhangYimingSelfProof = (reply: string, message: string) => {
  ...
  const sentences = normalizeText(reply)
    .split(/(?<=[。！？])/u)
...
const polishZhangYimingReply = (reply: string, message = '') =>
  normalizeText(
