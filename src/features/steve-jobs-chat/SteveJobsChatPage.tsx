...
import {
  createSteveJobsChatLocalFallback,
  createSteveJobsChatMessageId,
  getSteveJobsChatDeveloperUnlimited,
  getSteveJobsChatInitialRemaining,
  getSteveJobsChatQuotaText,
  getSteveJobsChatStatusMeta,
  normalizeAssistantReply,
  normalizeUserInput,
  setSteveJobsChatStoredRemaining,
  STEVE_JOBS_CHAT_HEALTHCHECK_TIMEOUT_MS,
  STEVE_JOBS_CHAT_INPUT_MAX_LENGTH,
  STEVE_JOBS_CHAT_REQUEST_TIMEOUT_MS
} from './client';
import { getSteveJobsChatApiUrl, getSteveJobsChatSurface } from './runtime';
import {
  resolveSteveJobsChatServiceStatus,
  STEVE_JOBS_CHAT_DESCRIPTION,
  STEVE_JOBS_CHAT_TITLE,
  type SteveJobsChatApiResponse,
  type SteveJobsChatRole,
  type SteveJobsChatServiceStatus
} from './shared';
import './SteveJobsChatPage.css';

type ChatMessage = {
  id: string;
  role: SteveJobsChatRole;
  content: string;
};
...
export default function SteveJobsChatPage() {
  ...
  const [isDeveloperUnlimited] = useState(getSteveJobsChatDeveloperUnlimited);
  const [remaining, setRemaining] = useState(getSteveJobsChatInitialRemaining);
  const [isSending, setIsSending] = useState(false);
  const [serviceStatus, setServiceStatus] = useState<SteveJobsChatServiceStatus>('checking');
  const [error, setError] = useState('');
  const [statusNotice, setStatusNotice] = useState('');
  const surface = getSteveJobsChatSurface();
  ...
  const quotaText = getSteveJobsChatQuotaText(isDeveloperUnlimited, remaining);
  const statusMeta = getSteveJobsChatStatusMeta(serviceStatus);
  ...
  useEffect(() => {
    const apiUrl = getSteveJobsChatApiUrl();
    ...
    const timeoutId = window.setTimeout(() => controller.abort(), STEVE_JOBS_CHAT_HEALTHCHECK_TIMEOUT_MS);
    ...
    const checkApiHealth = async () => {
      try {
        ...
        const payload = (await response.json().catch(() => null)) as SteveJobsChatApiResponse | null;
        setServiceStatus(resolveSteveJobsChatServiceStatus(payload?.status, payload?.mode, 'api'));
      } catch {
        if (!cancelled) {
          setServiceStatus('offline');
        }
  ...
  const submitMessage = async () => {
    ...
    const userMessage: ChatMessage = {
      id: createSteveJobsChatMessageId('user'),
      role: 'user',
      content
    };
    ...
    let { replyText, nextStatus, nextNotice, shouldConsume } = createSteveJobsChatLocalFallback({
      content,
      apiUrl,
      isStaticPreview
    });
    ...
    if (apiUrl) {
      const controller = new AbortController();
      const timeoutId = window.setTimeout(() => controller.abort(), STEVE_JOBS_CHAT_REQUEST_TIMEOUT_MS);
      ...
      try {
        ...
        const payload = (await response.json()) as SteveJobsChatApiResponse;
        replyText = normalizeAssistantReply(payload.reply || replyText);
        nextStatus = resolveSteveJobsChatServiceStatus(payload.status, payload.mode, 'mock');
    ...
    setMessages((current) => [
      ...current,
      {
        id: createSteveJobsChatMessageId('assistant'),
        role: 'assistant',
        content: replyText
      }
    ]);
    ...
    if (shouldConsume && !isDeveloperUnlimited) {
      setRemaining((value) => {
        ...
        setSteveJobsChatStoredRemaining(next);
        return next;
      });
    }
      ...
      <main className="jobs-chat-page">
        ...
        <div className="jobs-chat-shell">
          ...
          <section className="jobs-chat-hero" aria-labelledby="jobs-chat-title">
            ...
            <p className="jobs-chat-subtitle">{STEVE_JOBS_CHAT_DESCRIPTION}</p>
          </section>
          ...
          <section className="jobs-chat-panel" aria-label="虚拟乔布斯对话区域">
            ...
            <form className="jobs-chat-form" onSubmit={handleSubmit}>
              ...
              <textarea
                ...
                placeholder=""
                rows={4}
                maxLength={STEVE_JOBS_CHAT_INPUT_MAX_LENGTH}
              ...
              <div className="jobs-chat-form__meta" id="jobs-chat-input-meta">
                ...
                <span>
                  {input.length}/{STEVE_JOBS_CHAT_INPUT_MAX_LENGTH}
                </span>
              </div>
