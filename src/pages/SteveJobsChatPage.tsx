...
} from '../config/steveJobsChatShared';
import { getSteveJobsChatApiUrl, getSteveJobsChatSurface } from '../config/steveJobsChatApi';
...
const getInitialRemaining = () => (getDeveloperUnlimited() ? STEVE_JOBS_CHAT_FREE_LIMIT : getStoredRemaining());
...
const getStatusMeta = (status: SteveJobsChatServiceStatus) => {
  switch (status) {
    case 'api':
...
const resolveServiceStatus = (
  status?: SteveJobsChatApiResponse['status'],
  mode?: SteveJobsChatApiResponse['mode'],
  fallbackStatus: Exclude<SteveJobsChatServiceStatus, 'checking'> = 'offline'
): Exclude<SteveJobsChatServiceStatus, 'checking'> => {
...
const createMessageId = (role: SteveJobsChatRole) => `${role}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
...
export default function SteveJobsChatPage() {
  ...
  const submitMessage = async () => {
    ...
    let replyText = normalizeAssistantReply(buildSteveJobsMockReply(content));
    let nextStatus: Exclude<SteveJobsChatServiceStatus, 'checking'> = apiUrl ? 'mock' : isStaticPreview ? 'preview' : 'offline';
